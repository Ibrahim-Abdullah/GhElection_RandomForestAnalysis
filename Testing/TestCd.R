...
preds <- as.data.frame(rbind(pAPC,pCPP,pNDC,pNPP,pPPP))
names(preds) = c("APC","CPP","NDC","NPP","PPP")
...
predshow <- reshape(preds,direction='long',
                    timevar='Prediction',v.names='Score',times=c('APC','CPP','NDC','NPP','PPP'),
                    varying=list(w=c('APC','CPP','NDC','NPP','PPP')))
